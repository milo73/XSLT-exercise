...
<xsl:stylesheet 	version="2.0" 
	...
	<xsl:param name="commandToPublishVersion" select="'V3.3'"/>
	<xsl:param name="superProfileVersion" select="'V1.3'"/>
	<xsl:param name="noContentFoundString" select="'No content found for: '"/>
	<!--+
	      | This stylesheet transforms a commandToPublish 3.1 composition message into a commandToPublish 3.3 superProfile.		 
		  +-->
	<xsl:template match="/">
		<xsl:apply-templates/>
	</xsl:template>
	<xsl:template match="commandToPublish">
		...
		<xsl:element name="{local-name()}">
			...
			<xsl:attribute name="version"><xsl:value-of select="$commandToPublishVersion"/></xsl:attribute>
			<xsl:apply-templates/>
		</xsl:element>
	</xsl:template>
	<!--+
	...
	<xsl:template match="outlet" mode="contentSelect">
		...
		<xsl:apply-templates select="../../../../outlet/node()[@aid = $aidRef]" mode="selectTree">
			<xsl:with-param name="lineType" select="../../lineType/text()"/>
		</xsl:apply-templates>
	</xsl:template>
			...
			<xsl:when test="$totalContextTypes = 0 or $totalDerivedTypes = 0">
				<xsl:element name="telecom">
					...
					<xsl:apply-templates mode="dirtyHack">
						...
						<xsl:with-param name="areaCode" select="areaCode/text()" />
